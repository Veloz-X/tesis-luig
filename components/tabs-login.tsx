  ...
  const verificationEmail = async () => {
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/verifyEmail`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        ...
        toast.error("Error", {
          description:
            errorData.message || "El email no se verificó correctamente",
        });
      } else {
        const resrData = await res.json();
        console.log(resrData);
        toast.success("Email verificado", {
          description: "Se verificó el email correctamente",
        });
        setTabsValue("login");
      }
    } catch (error) {
      toast.error("Error", {
        description: "El email no se verificó correctamente",
      });
      ...
      <TabsContent value="check">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Ingresar el correo electrónico para verificar si existe en la base
              de datos
            </CardDescription>
          </CardHeader>
      ...
      <TabsContent value="login">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Ingresar el correo electrónico, la contraseña y codigo de
              verificación
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
